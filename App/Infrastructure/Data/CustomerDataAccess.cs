...
public static class CustomerDataAccess
{
    public static async Task AddCustomer(Customer customer)
    {
        ...
        var firstNameParameter = new SqlParameter(CustomerDataAccessConstants.FirstNameParam, SqlDbType.VarChar, 50)
            {Value = customer.Firstname};
        command.Parameters.Add(firstNameParameter);
        var surnameParameter = new SqlParameter(CustomerDataAccessConstants.SurnameParam, SqlDbType.VarChar, 50)
            {Value = customer.Surname};
        command.Parameters.Add(surnameParameter);
        var dateOfBirthParameter = new SqlParameter(CustomerDataAccessConstants.DateOfBirthParam, SqlDbType.DateTime)
            {Value = customer.DateOfBirth};
        command.Parameters.Add(dateOfBirthParameter);
        var emailAddressParameter =
            new SqlParameter(CustomerDataAccessConstants.EmailAddressParam, SqlDbType.VarChar, 50)
                {Value = customer.EmailAddress};
        command.Parameters.Add(emailAddressParameter);
        var hasCreditLimitParameter = new SqlParameter(CustomerDataAccessConstants.HasCreditLimit, SqlDbType.Bit)
            {Value = customer.HasCreditLimit};
        command.Parameters.Add(hasCreditLimitParameter);
        var creditLimitParameter = new SqlParameter(CustomerDataAccessConstants.CreditLimitParam, SqlDbType.Int)
            {Value = customer.CreditLimit};
        command.Parameters.Add(creditLimitParameter);
        var companyIdParameter = new SqlParameter(CustomerDataAccessConstants.CompanyIdParam, SqlDbType.Int)
